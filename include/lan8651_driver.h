...
typedef struct
{
   union MacAddr addr;    ///<MAC address
...
struct _NetInterface
{
   uint8_t          index;                        ///<Zero-based index
   uint32_t         id;                           ///<A unique number identifying the interface
   uint64_t         eui64;                        ///<EUI-64 interface identifier
   ...
   MacFilterEntry   macAddrFilter[12];            ///<MAC filter table
   union MacAddr    macAddr;                      ///<Link-layer address   
};
...
struct _NetRxAncillary
{
   ...
   union MacAddr srcMacAddr;     ///<Source MAC address
   union MacAddr destMacAddr;    ///<Destination MAC address
